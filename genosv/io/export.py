...
class TrackCompositor(object):
    ...
    def getBounds(self, tracks, allele):
        ...
        if len(self.dataHub.variant.chrom_parts(allele).parts) > 1:
            return 0, scale.pixelWidth

        segments = list(self.dataHub.variant.chrom_parts(allele))[0].segments
        alleleLength = len(list(self.dataHub.variant.chrom_parts(allele))[0])
        ...
        if True:
            ...
            for name, track in alleleTracks.items():
                ...
                track.render(scaleFactor=scaleFactor, height=imageHeight,  
                    thickerLines=False)#self.dataHub.args.thicker_lines)
